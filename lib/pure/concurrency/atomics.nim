...
## ---------------------
## Lock-free eligibility depends on type size and memory manager:
##
...
##
## Use `isLockFree(T)` to check at compile-time whether a type uses
## lock-free operations.
##
## To enforce lock-free operations (compile error instead of spinlock fallback),
## compile with `-d:nimEnforceLockFreeAtomics`.
##
...
template isLockFree*(T: typedesc): bool =
  ...
  # Note: Can't use `sizeof(T) in {1,2,4,8}` because sets are limited to 0..255
  (sizeof(T) == 1 or sizeof(T) == 2 or sizeof(T) == 4 or sizeof(T) == 8) and not isManaged(T)
  ...
  when defined(nimEnforceLockFreeAtomics):
    when not (sizeof(T) == 1 or sizeof(T) == 2 or sizeof(T) == 4 or sizeof(T) == 8):
      {.error: "Atomic[" & $T & "] cannot use lock-free atomics: sizeof(" & $T & ") = " &
               $sizeof(T) & " is not a valid atomic size (must be 1, 2, 4, or 8). " &
...
when (defined(cpp) and defined(nimUseCppAtomics)) or defined(nimdoc):
  ...
  template nonAtomicType*(T: typedesc): untyped =
    ...
    when sizeof(T) == 1: int8
      ...
      {.error: "nonAtomicType only supports types of size 1, 2, 4, or 8 bytes".}
  ...
  when defined(vcc):
    ...
    proc store*[T](location: var Atomic[T]; desired: T; order: MemoryOrder = moSequentiallyConsistent) {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        discard interlockedExchange(location.value.addr, cast[T.nonAtomicType](desired))
      else:
        withLockVcc(location, order):
    ...
    proc exchange*[T](location: var Atomic[T]; desired: T; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        cast[T](interlockedExchange(location.value.addr, cast[int64](desired)))
      else:
        withLockVcc(location, order):
          result = location.nonAtomicValue
          location.nonAtomicValue = desired
    ...
    proc compareExchange*[T](location: var Atomic[T]; expected: var T; desired: T; success, failure: MemoryOrder): bool {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        cast[T](interlockedCompareExchange(location.value.addr, cast[T.nonAtomicType](desired), cast[T.nonAtomicType](expected))) == expected
      else:
    ...
    proc load*[T](location: var Atomic[T]; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        cast[T](atomic_load_explicit[T.nonAtomicType, typeof(location.value)](location.value.addr, order))
      else:
    ...
    proc store*[T](location: var Atomic[T]; desired: T; order: MemoryOrder = moSequentiallyConsistent) {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        atomic_store_explicit(location.value.addr, cast[T.nonAtomicType](desired), order)
      else:
        withLock(location, order):
          location.nonAtomicValue = desired
    ...
    proc exchange*[T](location: var Atomic[T]; desired: T; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        cast[T](atomic_exchange_explicit(location.value.addr, cast[T.nonAtomicType](desired), order))
      else:
        withLock(location, order):
          result = location.nonAtomicValue
          location.nonAtomicValue = desired
    ...
    proc compareExchange*[T](location: var Atomic[T]; expected: var T; desired: T; success, failure: MemoryOrder): bool {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        atomic_compare_exchange_strong_explicit(location.value.addr, cast[ptr T.nonAtomicType](expected.addr), cast[T.nonAtomicType](desired), success, failure)
      else:
    ...
    proc compareExchangeWeak*[T](location: var Atomic[T]; expected: var T; desired: T; success, failure: MemoryOrder): bool {.inline.} =
      enforceLockFreeCheck(T)
      when isLockFree(T):
        atomic_compare_exchange_weak_explicit(location.value.addr, cast[ptr T.nonAtomicType](expected.addr), cast[T.nonAtomicType](desired), success, failure)
      else:
        compareExchange(location, expected, desired, success, failure)
    ...
    # Numerical operations
    proc fetchAdd*[T: SomeInteger](location: var Atomic[T]; value: T; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      cast[T](atomic_fetch_add_explicit(location.value.addr, cast[T.nonAtomicType](value), order))
    proc fetchSub*[T: SomeInteger](location: var Atomic[T]; value: T; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      cast[T](atomic_fetch_sub_explicit(location.value.addr, cast[T.nonAtomicType](value), order))
    proc fetchAnd*[T: SomeInteger](location: var Atomic[T]; value: T; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      cast[T](atomic_fetch_and_explicit(location.value.addr, cast[T.nonAtomicType](value), order))
    proc fetchOr*[T: SomeInteger](location: var Atomic[T]; value: T; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      cast[T](atomic_fetch_or_explicit(location.value.addr, cast[T.nonAtomicType](value), order))
    proc fetchXor*[T: SomeInteger](location: var Atomic[T]; value: T; order: MemoryOrder = moSequentiallyConsistent): T {.inline.} =
      cast[T](atomic_fetch_xor_explicit(location.value.addr, cast[T.nonAtomicType](value), order))
