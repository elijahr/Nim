...
##
## **Safe usage requires algorithms designed for memory reclamation:**
##
## | Memory Manager     | Pointer Swap | Reclamation     | Notes |
## |--------------------|--------------|-----------------|-------------------------------|
## | `--mm:refc`        | Lock-free    | Thread-local GC | Each thread has its own heap; cannot share refs |
## | `--mm:markAndSweep`| Lock-free    | Thread-local GC | Each thread has its own heap; cannot share refs |
## | `--mm:arc/orc`     | Blocked      | Non-atomic RC   | Refcount races; use `--mm:atomicArc` instead |
## | `--mm:atomicArc`   | Lock-free    | Atomic RC       | Use `GC_ref`/`GC_unref`; algorithm must prevent use-after-free |
## | `--mm:boehm`       | Lock-free    | Tracing GC      | Shared heap; pauses all threads during collection, latency spikes |
## | `--mm:go`          | Lock-free    | Tracing GC      | Shared heap; pauses all threads during collection, latency spikes |
## | `--mm:none`        | Lock-free    | Manual          | You manage all allocation and deallocation |
##
## **For `--mm:atomicArc`:** Call `GC_ref` when inserting into a structure
## (so the structure holds a reference). When removing, ownership transfers
## to the caller (no `GC_unref` needed; normal scoping handles it). The
## algorithm must ensure no thread accesses a node after it could be freed.
