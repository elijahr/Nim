discard """
  targets: "cpp"
  description: "Test deferred size/align pragma expressions for generic imported types"
"""
...
# alignof(T) - type inherits alignment from parameter
type
  AlignedWrapper[T] {.importcpp: "AlignedType", size: sizeof(T), align: alignof(T), completeStruct.} = object

static:
  doAssert sizeof(AlignedWrapper[int8]) == 1
  doAssert alignof(AlignedWrapper[int8]) == alignof(int8)
  doAssert sizeof(AlignedWrapper[int64]) == 8
  doAssert alignof(AlignedWrapper[int64]) == alignof(int64)

# sizeof and alignof together
type
  FullWrapper[T] {.importcpp: "FullType", size: sizeof(T), align: alignof(T), completeStruct.} = object

static:
  doAssert sizeof(FullWrapper[int32]) == 4
  doAssert alignof(FullWrapper[int32]) == alignof(int32)
  doAssert sizeof(FullWrapper[int64]) == 8
  doAssert alignof(FullWrapper[int64]) == alignof(int64)
...
# Verify regular align pragma on types works (non-generic, fixed align)
type
  FixedAlign {.importcpp, size: 8, align: 16, completeStruct.} = object

static:
  doAssert sizeof(FixedAlign) == 8
  doAssert alignof(FixedAlign) == 16

echo "All deferred size/align pragma tests passed!"
