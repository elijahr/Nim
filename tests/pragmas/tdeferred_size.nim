...
# -----------------------------------------------------------------------------
# Enum types with size pragma (non-deferred - fixed size only)
# Note: Deferred size expressions in enums require importc, so we test fixed sizes
# -----------------------------------------------------------------------------

type FixedSizeEnum {.size: 2.} = enum
  fseOne, fseTwo, fseThree

static:
  doAssert sizeof(FixedSizeEnum) == 2

# -----------------------------------------------------------------------------
# Mixed size and align pragmas on same type
# -----------------------------------------------------------------------------

type MixedPragmas[T] {.importc, size: sizeof(T), align: alignof(T), completeStruct.} = object

static:
  doAssert sizeof(MixedPragmas[int8]) == 1
  doAssert sizeof(MixedPragmas[int32]) == 4
  doAssert sizeof(MixedPragmas[int64]) == 8
  doAssert alignof(MixedPragmas[int64]) >= alignof(int64)
