...
- `std/atomics` now determines lock-free eligibility based on `sizeof(T) <= sizeof(pointer)`
  instead of a fixed type list. This means:
  ...
  - With non-destructor MMs (`--mm:refc`, `--mm:markAndSweep`, `--mm:none`), managed types
    like `ref`, `string`, `seq` are pointer-sized and can use lock-free atomics
  - With destructor-based MMs (`--mm:arc`, `--mm:orc`, `--mm:atomicArc`), `supportsCopyMem(T)`
    must be true (excludes managed types)
